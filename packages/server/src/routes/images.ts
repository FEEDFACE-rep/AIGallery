...
// Image delete
//imagesRouter.post('/images', async (req:any, res) => {
//  if (req.query.delete !== undefined && req.query.filename !== undefined)
//  {
//    const serverFileName = await deleteImageIntoDB(req.query.filename);
//    res.status(200).json({ status: 'ok', error: 'File deleted.' });
//    return;
//  }
//    res.status(400).json({ status: 'error', error: 'Wrong query' });
//});

// Image upload
imagesRouter.post('/', async (req:any, res) => {
  if (req.query.del !== undefined && req.query.filename !== undefined)
  {
    await delImage(req.query.filename);
    res.status(200).json({ status: 'ok', error: 'File deleted.' });
  }
